...
const VERSION_FILE_PATHS = Object.fromEntries(
  VERSION_RELATIVE_PATHS.map((relativePath) => [
    relativePath,
...
describe("版本清单", () => {
  ...
  it.each([
    ...
    ["packageJson", "package.json", "[]"],
    ["packageJson", "package.json", '"apix"'],
    ["packageLock", "package-lock.json", "null"],
    ["tauriConfig", "src-tauri/tauri.conf.json", "null"],
    ["tauriConfig", "src-tauri/tauri.conf.json", "[]"],
    ["tauriConfig", "src-tauri/tauri.conf.json", '"apix"'],
  ...
  it("更新五个版本源的版本字段", () => {
    ...
    expect(JSON.parse(updated.packageJson).version).toBe("0.1.1");
    expect(JSON.parse(updated.packageLock).version).toBe("0.1.1");
    expect(JSON.parse(updated.packageLock).packages[""].version).toBe("0.1.1");
    expect(JSON.parse(updated.tauriConfig).version).toBe("0.1.1");
    expect(updated.cargoToml).toContain('version = "0.1.1"');
    expect(updated.cargoLock).toContain('version = "0.1.1"');
  });
...
function releaseHarness(
  ...
  const files = new Map([
    [VERSION_FILE_PATHS["package.json"], manifests.packageJson],
    [VERSION_FILE_PATHS["package-lock.json"], manifests.packageLock],
    [VERSION_FILE_PATHS["src-tauri/tauri.conf.json"], manifests.tauriConfig],
    [VERSION_FILE_PATHS["src-tauri/Cargo.toml"], manifests.cargoToml],
    [VERSION_FILE_PATHS["src-tauri/Cargo.lock"], manifests.cargoLock],
  ]);
  const calls = [];
  const events = [];
  ...
  const execute = (command, commandArgs) => {
    ...
    if (key === "git remote get-url origin") return "git@github.com:owner/apix.git";
...
describe("发布编排", () => {
  it("夹具文件键与 runRelease 使用相同的 path.join 规则", () => {
    const state = {};
    releaseHarness(["--current"], { state });

    for (const relativePath of VERSION_RELATIVE_PATHS) {
      expect(state.files.has(path.join(REPO_ROOT, relativePath))).toBe(true);
    }
  });
  ...
  it("Windows 直接执行 npm.exe 并保留参数数组", () => {
    const runtime = {
      platform: "win32",
      nodePath: "C:\\Program Files\\nodejs\\node.exe",
      npmExecPath: "C:\\Program Files\\nodejs\\npm.exe",
    ...
    expect(calls).toContainEqual([runtime.npmExecPath, "run", "build"]);
    expect(calls.some(([command]) => command === runtime.nodePath)).toBe(false);
  });

  it.each(["C:\\nodejs\\npm.cmd", "C:\\nodejs\\npm.bat"])(
    "Windows 拒绝批处理 npm 入口 %s",
    (npmExecPath) => {
      expect(() =>
        releaseHarness([], {
          runtime: {
  ...
  it("Windows 缺少 npm_execpath 时给出明确错误", () => {
    ...
    ).toThrow("无法定位 npm 入口：缺少 npm_execpath");
  });
  ...
  it("校验、提交并推送新版本和标签", () => {
    ...
    expect(calls).toContainEqual(["git", "commit", "-m", "chore: release v0.1.1"]);
  ...
  it("严格保护普通发布的安全执行顺序", () => {
    ...
    expect(relevantEvents).toEqual([
      ...
      ["write", VERSION_FILE_PATHS["package.json"]],
      ["write", VERSION_FILE_PATHS["package-lock.json"]],
      ["write", VERSION_FILE_PATHS["src-tauri/tauri.conf.json"]],
      ["write", VERSION_FILE_PATHS["src-tauri/Cargo.toml"]],
      ["write", VERSION_FILE_PATHS["src-tauri/Cargo.lock"]],
      [
        "command",
        "cargo",
        "metadata",
      ...
      ["command", "git", "commit", "-m", "chore: release v0.1.1"],
      ["command", "git", "push", "origin", "main"],
      ["command", "git", "tag", "-a", "v0.1.1", "-m", "发布 APIX v0.1.1"],
  ...
  it("current 不写版本或提交并强推当前标签", () => {
    ...
    expect(files.get(VERSION_FILE_PATHS["package.json"])).toBe(manifests.packageJson);
  ...
  it("提交失败时撤销暂存并恢复版本文件", () => {
    const state = {};
    expect(() =>
      releaseHarness([], { failOn: "git commit -m chore: release v0.1.1", state }),
    ).toThrow("模拟失败");
    ...
    expect(state.files.get(VERSION_FILE_PATHS["package.json"])).toBe(
      manifests.packageJson,
    );
    expect(state.files.get(VERSION_FILE_PATHS["package-lock.json"])).toBe(
      manifests.packageLock,
    );
    expect(state.files.get(VERSION_FILE_PATHS["src-tauri/tauri.conf.json"])).toBe(
      manifests.tauriConfig,
    );
    expect(state.files.get(VERSION_FILE_PATHS["src-tauri/Cargo.toml"])).toBe(
      manifests.cargoToml,
    );
    expect(state.files.get(VERSION_FILE_PATHS["src-tauri/Cargo.lock"])).toBe(
      manifests.cargoLock,
    );
  });
