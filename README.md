...
- **错误日志**：可开启报错收集，在响应区查看、复制、清空 HTTP / WS / SSE 调试错误。日志入库前会隐藏请求头值、URL 凭据与查询值、原始消息载荷及常见认证字段。
...
- **界面体验**：支持明暗主题、侧栏宽度调整、请求/响应区域高度调整、响应区展开，关闭后重新打开会恢复上次窗口大小及最大化状态，以及 `⌘ + Enter` / `Ctrl + Enter` 发送请求。
...
- HTTP 响应与历史记录绑定到发起请求时的接口；等待响应期间切换项目，不会把原请求的响应写入新选中的接口。

### 重定向

HTTP 请求仅自动跟随同源重定向（协议、主机和端口均相同），最多 10 次；跨源跳转和 HTTPS 降级会被阻止，重定向不自动附带 Referer。SSE 不自动跟随重定向，遇到 3xx 会提示直接使用目标地址，以免认证信息被转发到非预期服务器。
...
导入和导出会移除已不再使用的旧版 `auth` 配置。项目 JSON 用于备份与迁移，仍包含全部环境配置、请求及响应的真实值。
...
项目可导出为接口文档 HTML，并支持选择导出的模块。文档中的 URL、Headers、Query 参数、结构化 Body 及复制 URL / cURL 的内容会对常见敏感字段脱敏；上传文件仅保留文件名。最近一次响应仅保留状态码和耗时，真实响应头、响应体及无法结构化处理的请求体会省略。导出不会修改项目中的原始数据。
